...
class Mesh():
    ...
    def contour_plot(self, ver, f, a):
        ...
        a0 = a[0].contourf(X, Y, ccc2.reshape(np.shape(X)), 40, cmap = 'BuPu')
        a1 = a[1].contourf(X, Y, ddd2.reshape(np.shape(X)), 40, cmap = 'BuPu')
    ...
    def error_plot(self, Dis, f, a):
        ...
        DiffX = 100*np.divide(abs(TrueX - EstimateX), TrueX + 1e-20)

        DiffY = 100*np.divide(abs(TrueY - EstimateY), TrueY + 1e-20)
        ...
        a0 = a[0].contourf(X, Y, DiffX.reshape(np.shape(X)), 40, cmap = 'BuPu')
        a1 = a[1].contourf(X, Y, DiffY.reshape(np.shape(X)), 40, cmap = 'BuPu')
