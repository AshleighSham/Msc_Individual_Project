...
colours = ['deepskyblue','mediumseagreen','orange','hotpink','mediumorchid','mediumvioletred']
labels = ['MH', 'AMH', 'DR MH', 'DRAM', 'pCN', 'EnKF']
fig, ax = plt.subplots(2,1, figsize = (10,5) )

for i, j, q in zip(read_dictionary['graphs'][0], read_dictionary['values'][0], range(6)):
    ax[0].plot(i[0], i[1], alpha = 0.8, c = colours[q])
    ax[0].plot([j], [0], c = colours[q], marker = "|", markersize=18, markeredgewidth = 3)

for i, j, q in zip(read_dictionary['graphs'][1], read_dictionary['values'][1], range(6)):
...
fig.legend(loc='lower center', ncol = 7, prop={'size': 20})
ax[0].grid()
