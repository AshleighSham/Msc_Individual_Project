...
for i in range(len(measurements1)//2):
    x.append(measurements1[2*i])
    y.append(measurements1[2*i + 1])
...
print('The median of the Yield Stress posterior is: %f, with uncertainty +/- %.5f' % (np.median(results['MCMC'][1]), np.sqrt(np.var(results['MCMC'][1]))))
print('The median of the Hardening Modulus posterior is: %f, with uncertainty +/- %.5f' % (np.median(results['MCMC'][2]), np.sqrt(np.var(results['MCMC'][2]))))
print()

outi =np.array([[np.median(results['MCMC'][0])], [np.median(results['MCMC'][1])], [np.median(results['MCMC'][2])]])

measurements=utilities.forward_model(outi)
X,Y = [], []
for i in range(len(measurements)//2):
