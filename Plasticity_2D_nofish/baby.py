    ...
    def Baby_go(self):
        ...
        while j < self.nsamples:
            ...
            if j % 100 == 0:
                ...
                print('The median of the Yield Stress posterior is: %f, with uncertainty +/- %.5f' % (np.median(self.results['MCMC'][1][:j]), np.sqrt(np.var(self.results['MCMC'][1][:j]))))
                print('The median of the Hardening Modulus posterior is: %f, with uncertainty +/- %.5f' % (np.median(self.results['MCMC'][2][:j]), np.sqrt(np.var(self.results['MCMC'][2][:j]))))
                print()
