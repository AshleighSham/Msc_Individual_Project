...
def forward_model(params, mesh):
    ...
    youngs_modulus_i, poisson_ratio_i, yield_strs_i, hardening_modulus_i, youngs_modulus_m, poisson_ratio_m, yield_strs_m, hardening_modulus_m = params[:,0]
    ...
    my_mesh = Mesh(mesh)
    node_coord, elem_conn, elem_spacing, thickness, restrained_dof, force_ext_dof, force_ext_mag, element_IDS, thickness = my_mesh.meshgrid()
    ...
    hardening_pt_yield_strs_i = np.array([yield_strs_i, yield_strs_i + hardening_modulus_i])
    hardening_pt_yield_strs_m = np.array([yield_strs_m, yield_strs_m + hardening_modulus_m])
    ...
    if sol_alg_case == "load control":
        ...
        disp_trgt_dof_id = force_ext_dof[0]
        # number of time steps per loading
        n_time_steps_per_loading = 50
        # max displacement target
        disp_trgt_max = 0.00005
        #disp_trgt_max = -0.02
        # (loading only)
    ...
    # (for plotting)disp_trgt_dof_id
    # store degree of freedom id
    strg_dof_id = np.array([disp_trgt_dof_id, force_ext_dof[1]])
    # store node id
    ...
    disp_I_strg, disp_I_strg_n, force_ext_I_strg, Cauchy_strn_tnr_strg, Cauchy_strn_elc_tnr_strg, \
    ...
    while i_time_step < n_time_steps:
        ...
        while i_NR_iter < n_max_NR_iters and has_convrged == False:
            ...
            stiff_mtrx_IJ_tp1 = get_stiff_mtrx_IJ_tp1_2d(example_case, n_elems, elem_dof, n_dofs_per_elem, n_integr_pts_per_elem, \
                                        ...
                                        integr_pt_plc_const_mtrx_flag_tp1, youngs_modulus_i, youngs_modulus_m, poisson_ratio_i, poisson_ratio_m, \
                                        hardening_pt_eqn_plc_strn, hardening_pt_yield_strs_i, hardening_pt_yield_strs_m, \
                                        n_dofs, integr_pt_vol, element_IDS, thickness)
            ...
            integr_pt_Cauchy_strs_eqn_tp1, integr_pt_has_yielded_tp1, integr_pt_sufail_tp1, force_int_I_ttp1_inc = \
                    ...
                    integr_pt_Cauchy_strn_plc_tnr_t, integr_pt_Cauchy_strn_plc_eqn_t, youngs_modulus_i, youngs_modulus_m, poisson_ratio_i, poisson_ratio_m,
                    hardening_pt_eqn_plc_strn, hardening_pt_yield_strs_i, hardening_pt_yield_strs_m, integr_pt_Cauchy_strs_tnr_t,
                    integr_pt_vol, element_IDS)
    ...
    for i in range(len(disp_I_strg)):
        ...
        line.append([1000*disp_I_strg_n[i]])
