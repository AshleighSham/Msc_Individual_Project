...
class Mesh():
    ...
    def __init__(self, m):
        ...
        self.BCnodes = np.array([0, 1, 8, 9, 16, 17, 24, 25])

        frn = []
        for i in range(len(self.DOF2[0])//2):
            frn.append(self.DOF2[-1][2*i + 1])
        self.forcenodes = np.array([6, 14, 22, 30])
        self.forces = np.array([1, 1, 1, 1])*1e-3

        self.IDs = EID

    def meshgrid(self):
        return self.XYZ, self.CON + 1, self.scfa, self.th, self.BCnodes + 1, self.forcenodes + 1, self.forces, self.IDs, self.th
