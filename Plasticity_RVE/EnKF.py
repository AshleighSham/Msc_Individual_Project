...
class EnKF_mcmc():
    ...
    def save_data(self, j):
        ...
        df.to_csv(r'C:\Users\ashle\Documents\GitHub\Portfolio\ES98C\Plasticity_RVE\EnKF.csv', mode='a', index=True, header = False)

    def Kalman_gain(self, j):
        a = self.m0**2*np.var(self.observations[0::3])
        b = self.m0**2*np.var(self.observations[1::3])
        c = self.m0**2*np.var(self.observations[2::3])
    ...
    def EnKF_go(self):
        ...
        while j < self.s:
            ...
            Noise[0::3] = np.random.normal(0, self.m0*np.sqrt(np.var(self.observations[0::3])), size = np.shape(self.observations[0::3]))
            Noise[1::3] = np.random.normal(0, self.m0*np.sqrt(np.var(self.observations[1::3])), size = np.shape(self.observations[1::3]))
            Noise[2::3] = np.random.normal(0, self.m0*np.sqrt(np.var(self.observations[2::3])), size = np.shape(self.observations[2::3]))
            
            dt = KK @ (- self.oldvalue + self.observations + Noise)
