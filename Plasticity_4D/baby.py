...
class Baby_mcmc():
    ...
    def Baby_go(self):
        ...
        while j < self.nsamples:
            ...
            #     print('The median of the Yield Stress posterior is: %f, with uncertainty +/- %.5f' % (np.median(self.results['MCMC'][2][:j]), np.sqrt(np.var(self.results['MCMC'][2][:j]))))
            #     print('The median of the Hardening Modulus posterior is: %f, with uncertainty +/- %.5f' % (np.median(self.results['MCMC'][3][:j]), np.sqrt(np.var(self.results['MCMC'][3][:j]))))
            #     print()
