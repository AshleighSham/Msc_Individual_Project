...
class Mesh():
    ...
    def deformation_plot(self, label, colour, ch, ax, lines, ls, D = np.array([1]), non = True):
        ...
        #figure = plt.figure()
        if non == True:
            ax.plot(self.XYZ[:,0], self.XYZ[:, 1],'sk', markersize='1', zorder = 1, alpha = 0.6)
        ax.scatter(self.XYZ[:,0] + D[0:len(D):2].reshape(-1), self.XYZ[:,1] + D[1:len(D):2].reshape(-1), c = colour ,s=10, label = label, zorder = 5, alpha = ch)
        #plt.title(title)
