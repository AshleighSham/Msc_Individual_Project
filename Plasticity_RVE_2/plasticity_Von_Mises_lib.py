...
def get_stiff_mtrx_IJ_tp1_2d(example_case, n_elems, elem_dof, n_dofs_per_elem, n_integr_pts_per_elem, \
                            ...
                            n_dofs, integr_pt_vol, ele_IDS, thickness):
    ...
    for i_elem in range(n_elems):
        if ele_IDS[i_elem] == 0:
            youngs_modulus = youngs_modulus_i
            poisson_ratio = poisson_ratio_i
            hardening_pt_yield_strs = hardening_pt_yield_strs_i
        elif ele_IDS[i_elem] == 1:
            youngs_modulus = youngs_modulus_m
            poisson_ratio = poisson_ratio_m
            hardening_pt_yield_strs = hardening_pt_yield_strs_m
        # element connectivity
...
def get_fld_integr_pt_strg(n_time_steps, n_strns_per_integr_pt, \
    ...
    disp_I_strg_n = np.zeros(n_time_steps)
    ...
    # return
    return disp_I_strg, disp_I_strg_n, force_ext_strg, Cauchy_strn_tnr_strg, \
...
def store_fld_integr_pt_strg(i_time_step, dof_id, integr_pt_id, \
    ...
    # control node id
    disp_I_strg[i_time_step - 1] = disp_I_tp1[dof_id[0]-1]
    force_ext_I_strg[i_time_step - 1] = force_ext_I_tp1[dof_id[0]-1]   

    # second node
    disp_I_strg_n[i_time_step - 1] = disp_I_tp1[dof_id[1]-1]
    ...
    return disp_I_strg, disp_I_strg_n, force_ext_I_strg, Cauchy_strn_tnr_strg, Cauchy_strn_elc_tnr_strg, \
        Cauchy_strn_plc_tnr_strg, Cauchy_strn_plc_eqn_strg, Cauchy_strs_tnr_strg, \
        Cauchy_strs_eqn_strg, has_yielded_strg, sufail_strg
