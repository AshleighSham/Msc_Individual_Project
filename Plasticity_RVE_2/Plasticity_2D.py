...
for i in range(len(inp['measurement'])//2):
    ...
    c.append(inp['measurement'][2*i + 1]/1000)

x1,yy = [], []
for i in range(len(measurements)//2):
    x1.append(measurements[2*i]/1000)
    yy.append(measurements[2*i + 1]/1000)

axd.scatter(a, c, label = 'Noisy Data', s = 40, linewidth = 1.5, marker = 'x', color = 'black', zorder = 10)
axd.plot(x1, yy, label = 'True Data', linewidth = 2)
axd.legend()
plt.grid()
axd.set(xlabel='Displacement (mm)', ylabel = 'External Force (N)')
# plt.show()
...
X1, YY = [], []
for i in range(len(measurements)//2):
    X1.append(measurements2[2*i]/1000)
    YY.append(measurements2[2*i + 1]/1000)

print(f'RMSE: {np.linalg.norm(measurements2/1000 - measurements/1000)/len(measurements)}')

axd.plot(X1, YY, label = 'MCMC result')
